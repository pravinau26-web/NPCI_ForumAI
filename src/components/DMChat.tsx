...
import { 
  ...
  Circle, Terminal, RefreshCw, Cpu, Users, Paperclip, X, FileText, Image as ImageIcon, Trash2
} from "lucide-react";
...
interface DMChatProps {
  ...
  onAddMembersSuccess?: (updatedChat: Chat) => void;
...
export default function DMChat({
  ...
  const nonMembers = users.filter(
    (u) => u.id !== "npci_assistant" && !activeChat?.participants.includes(u.id)
  );

  const handleAddMembers = async () => {
    if (!activeChat || selectedNewMemberIds.length === 0) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (onAddMembersSuccess) onAddMembersSuccess(updatedChat);
        setSelectedNewMemberIds([]);
        setShowAddMemberForm(false);
      } else {
        const err = await res.json();
        alert(err.error || "Failed to add group members.");
      }
    } catch (e) {
      console.error(e);
      alert("Network error while adding group members.");
    } finally {
                  ...
                  <div className="w-full max-w-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl py-4 px-5 text-left animate-in zoom-in-95 duration-150">
                    ...
                    <div className="max-h-[50vh] overflow-y-auto space-y-1 pr-1 scrollbar-thin">
                      {groupMembers.map(member => {
                        const isGroupCreator = activeChat.creatorId === currentUser.id || (activeChat.participants.length > 0 && activeChat.participants[0] === currentUser.id);
                        const isGroupAdmin = currentUser.role === "platform_admin" || currentUser.role === "policy_admin";
                        const canManageGroupMembers = isGroupCreator || isGroupAdmin;

                        return (
                          <div key={member.id} className="flex items-center justify-between p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                            <div 
                              ...
                              <div className="flex-1 min-w-0 text-left">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">
                                    {member.username} {member.id === currentUser.id && <span className="text-[10px] text-slate-400 font-mono ml-0.5">(You)</span>}
                                  </p>
                                  <span className="text-[8px] bg-emerald-50 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 font-extrabold px-1.5 py-0.5 rounded font-mono border border-emerald-200 dark:border-emerald-800 shrink-0">
                                    🛡️ Verified Secure
                                  </span>
                                </div>
                            ...
                            {canManageGroupMembers && member.id !== currentUser.id && (
                              <button
                                type="button"
                                ...
                                onClick={async (e) => {
                                  ...
                                  if (confirm(`Remove ${member.username} from this group?`)) {
                                    try {
                                      const res = await fetch(`/api/chats/${activeChat.id}/members/${member.id}`, {
                                        method: "DELETE",
                                        headers: { "x-user-id": currentUser.id }
                                      });
                                      if (res.ok) {
                                        const data = await res.json();
                                        if (onAddMembersSuccess) onAddMembersSuccess(data.chat);
                                      } else {
                                        const err = await res.json();
                                        alert(err.error || "Only group creator or Admin can remove members.");
                                      }
                                    } catch (err) {
                    ...
                    {/* Add Coworkers Drawer / Form */}
                    {showAddMemberForm ? (
                      <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 space-y-3 bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl animate-in zoom-in-95 duration-150">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-bold text-slate-700 dark:text-slate-200">
                            Select Coworkers to Add ({selectedNewMemberIds.length})
                          </span>
                          <button
                            type="button"
                        ...
                        {nonMembers.length === 0 ? (
                          <p className="text-xs text-slate-400 font-medium py-2">
                            All employees are already participants in this group.
                          </p>
                        ) : (
                          <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1">
                            {nonMembers.map((user) => {
                              ...
                              return (
                                <label
                                  key={user.id}
                                  className={`flex items-center gap-2.5 p-2 rounded-xl text-xs font-semibold cursor-pointer transition ${
                                    isSelected
                                ...
                                >
                                  <input
                                    ...
                                    onChange={(e) => {
                                      if (e.target.checked) {
                                        setSelectedNewMemberIds((prev) => [...prev, user.id]);
                                      } else {
                                  ...
                                  <img
                                    ...
                                    referrerPolicy="no-referrer"
                                  />
                                  <span className="truncate flex-1 font-semibold">{user.username}</span>
                                  <span className="text-[8px] bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 font-extrabold px-1.5 py-0.5 rounded font-mono shrink-0">
                                    🛡️ Verified Secure
                                  </span>
                                </label>
                              );
                        ...
                        <div className="pt-2 border-t border-slate-200/60 dark:border-slate-800/60">
                          <label className="flex items-center gap-2 cursor-pointer text-[11px] font-bold text-slate-700 dark:text-slate-300">
                            <input
                              ...
                              className="rounded text-blue-600 focus:ring-blue-500"
                            />
                            <span>Include prior chat history for new member(s)</span>
                          </label>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl transition shadow flex items-center justify-center gap-1.5 disabled:opacity-50 cursor-pointer"
                        >
                          {isAddingMembers ? (
                            <>
                      ...
                      <button
                        ...
                        className="mt-3 w-full py-2 bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/40 dark:hover:bg-blue-900/60 text-blue-600 dark:text-blue-300 font-bold text-xs rounded-xl transition flex items-center justify-center gap-1.5 cursor-pointer border border-blue-200 dark:border-blue-900"
                      >
                        <Users className="w-4 h-4 text-blue-500" />
                        <span>+ Add Coworkers to Group</span>
                      </button>
                    )}
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-2">
                      ...
                      {(activeChat.creatorId === currentUser.id || (activeChat.participants.length > 0 && activeChat.participants[0] === currentUser.id) || currentUser.role === "platform_admin" || currentUser.role === "policy_admin") && (
                        <button
                          onClick={async () => {
                            if (confirm("Are you sure you want to permanently delete this group?")) {
                              try {
                                const res = await fetch(`/api/chats/${activeChat.id}?actorId=${currentUser.id}`, {
                                  method: "DELETE",
                                  headers: { "x-user-id": currentUser.id }
                                });
                                if (res.ok) {
