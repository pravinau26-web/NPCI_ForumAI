...
import { 
  ...
  Search, Star, X, CheckSquare, Square, UserPlus, ShieldCheck, Mail, Trash2
} from "lucide-react";
          ...
          <div className="space-y-3">
              ...
              <div className="space-y-1">
                {communities.map((comm) => {
                  ...
                  const isSystemDefaultComm = ["comm-1", "comm-2", "comm-3", "comm-4"].includes(comm.id);
                  const canDelete = onDeleteCommunity && !isSystemDefaultComm && (
                    currentUser.role === "platform_admin" || 
                    comm.createdBy === currentUser.id
                  );
                  ...
                  return (
                    <div
                      key={comm.id}
                      className={`group w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-sm transition text-left ${
                        sidebarCollapsed ? "justify-center" : ""
                      } ${
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          {canDelete && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                if (confirm(`Are you sure you want to delete community #${comm.name}?`)) {
              ...
              {isPrivate && (
                <div className="space-y-3 p-3 bg-slate-50 dark:bg-slate-950/40 rounded-xl border border-slate-150 dark:border-slate-850 animate-in fade-in slide-in-from-top-2 duration-150 max-h-56 overflow-y-auto">
                  ...
                  <div className="border-t border-slate-200 dark:border-slate-800 pt-2">
                    <label className="block text-[9px] font-bold text-slate-400 mb-1 uppercase font-mono">
                      Allowed Individual Users & Security Status
                    </label>
                    <div className="space-y-1 max-h-28 overflow-y-auto pr-1">
                      {users
                        ...
                        .map(u => {
                          ...
                          return (
                            <label key={u.id} className="flex items-center justify-between text-xs text-slate-650 dark:text-slate-300 cursor-pointer select-none p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-900 transition">
                              <div className="flex items-center gap-1.5 truncate min-w-0">
                                <input
                                  ...
                                  className="w-3.5 h-3.5 rounded text-blue-500 focus:ring-blue-500 border-slate-300 dark:border-slate-850 shrink-0"
                                />
                                <span className="truncate font-medium">@{u.username}</span>
                              </div>
                              <span className="text-[9px] bg-emerald-50 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 font-extrabold px-1.5 py-0.5 rounded font-mono shrink-0 flex items-center gap-1 border border-emerald-200 dark:border-emerald-800">
                                🛡️ Verified Secure
                              </span>
                            </label>
                          );
                        ...
                        >
                          <div className="flex items-center gap-2 truncate min-w-0">
                            ...
                            <div className="truncate text-left">
                              ...
                              <span className="text-[10px] text-slate-400 ml-1">({member.email})</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <span className="text-[9px] bg-emerald-50 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 font-extrabold px-1.5 py-0.5 rounded font-mono flex items-center gap-1 border border-emerald-200 dark:border-emerald-800">
                              🛡️ Verified Secure
                            </span>
                            {isSelected ? (
