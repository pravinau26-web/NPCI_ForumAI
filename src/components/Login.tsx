...
import { Shield, Sparkles, UserCheck, Mail, UserPlus, Key, ArrowRight, Check, Activity, Lock, AlertCircle, AlertTriangle, HelpCircle, Sun, Moon, Eye, EyeOff } from "lucide-react";
...
export default function Login({ onLoginSuccess, theme, onToggleTheme }: LoginProps) {
  ...
  const [isSuspendedNotice, setIsSuspendedNotice] = useState(false);
      ...
      if (!response.ok) {
        ...
        try {
          ...
          try {
            ...
            errorData = { error: rawText && !rawText.startsWith("<") ? rawText : "Authentication failed (HTTP " + response.status + ")." };
          }
        } catch (readErr) {
          errorData = { error: "Authentication failed." };
        }

        if (errorData.isSuspended || (errorData.error && typeof errorData.error === 'string' && errorData.error.toLowerCase().includes("suspended"))) {
          setIsSuspendedNotice(true);
          setErrorMsg("");
          return;
        }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col lg:flex-row h-full transition-colors duration-200 relative overflow-x-hidden">
      ...
      <div className="flex-1 bg-slate-100 dark:bg-slate-950 p-8 lg:p-12 flex flex-col justify-center min-h-[500px]">
        <div className="max-w-md mx-auto w-full space-y-8 text-left">
          ...
          {isSuspendedNotice ? (
            <div className="p-4 bg-rose-500/10 dark:bg-rose-950/60 border-2 border-rose-500/40 rounded-2xl space-y-2 text-left animate-in fade-in duration-200">
              <div className="flex items-center gap-2 text-rose-600 dark:text-rose-400 font-bold text-sm">
                <AlertTriangle className="w-5 h-5 text-rose-500 shrink-0" />
                <span>ACCOUNT SUSPENDED</span>
              </div>
              <p className="text-xs text-rose-800 dark:text-rose-200 font-semibold leading-relaxed">
                Your workspace account has been suspended by a Platform Administrator. Login and feature access are currently restricted.
              </p>
              <div className="text-[11px] text-slate-500 dark:text-slate-400 font-mono pt-2 border-t border-rose-200/50 dark:border-rose-900/50 flex justify-between items-center">
                <span>Support: <strong>admin@npci.org.in</strong></span>
                <button 
                  type="button" 
                  onClick={() => setIsSuspendedNotice(false)}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase mb-1.5">
                Workspace Email Address (@npci.org.in)
              </label>
              <div className="relative">
                ...
                <input
                  type="email"
                  required
                  placeholder="e.g. pravinkumar.a@npci.org.in"
                  value={emailInput}
