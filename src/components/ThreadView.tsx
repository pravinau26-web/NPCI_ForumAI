                              ...
                              {comment.attachments && comment.attachments.length > 0 && (
                                <div className="mt-3 pt-2 border-t border-slate-100 space-y-2 max-w-sm">
                                  {comment.attachments.map((file, idx) => {
                                    ...
                                    return (
                                      <div key={idx} className="rounded-xl overflow-hidden border border-slate-200 shadow-xs group">
                                        {isImage ? (
                                          <div 
                                            className="bg-slate-100 dark:bg-slate-800 flex justify-center items-center relative cursor-pointer"
                                            onClick={() => onPreviewAttachment && onPreviewAttachment(file)}
                                          >
                                            <img
                                              src={file.url}
                                              alt={file.name}
                                              className="object-contain max-h-44 w-full group-hover:scale-[1.01] transition-transform duration-150"
                                            />
                                          ...
                                          <div
                                            onClick={() => onPreviewAttachment && onPreviewAttachment(file)}
                                            className="flex items-center justify-between p-2.5 text-xs bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition cursor-pointer"
                                          >
                                            <div className="flex items-center gap-2 truncate flex-1 min-w-0">
                                              <Paperclip className="w-4 h-4 text-blue-500 flex-shrink-0" />
                                              <div className="truncate text-left">
                                                <p className="font-semibold truncate leading-tight group-hover:text-blue-600 transition-colors">{file.name}</p>
                                                <p className="text-[10px] text-slate-400 font-mono mt-0.5">{file.size} • Click to Preview</p>
                                              </div>
                                            </div>
                                            <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold shrink-0 ml-2">Preview</span>
                                          </div>
                                        )}
                                      </div>
                                    );
                                  ...
                                  {reply.attachments && reply.attachments.length > 0 && (
                                    <div className="mt-2 pt-2 border-t border-slate-100 space-y-2 max-w-sm">
                                      {reply.attachments.map((file, idx) => {
                                        const isImage = file.type.startsWith("image/") || /\.(jpg|jpeg|png|webp|gif|svg)$/i.test(file.name);
                                        return (
                                          <div key={idx} className="rounded-lg overflow-hidden border border-slate-200 shadow-xs group">
                                            {isImage ? (
                                              <div 
                                                className="bg-slate-100 dark:bg-slate-800 flex justify-center items-center relative cursor-pointer"
                                                onClick={() => onPreviewAttachment && onPreviewAttachment(file)}
                                              >
                                                <img
                                                  src={file.url}
                                                  alt={file.name}
                                                  className="object-contain max-h-36 w-full group-hover:scale-[1.01] transition-transform duration-150"
                                                />
                                                <span className="absolute bottom-1 right-1 bg-slate-900/80 text-white text-[9px] px-1.5 py-0.5 rounded font-mono font-bold backdrop-blur-xs">
                                                  👁️ Preview
                                                </span>
                                              </div>
                                            ) : (
                                              <div
                                                onClick={() => onPreviewAttachment && onPreviewAttachment(file)}
                                                className="flex items-center justify-between p-2 text-xs bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition cursor-pointer"
                                              >
                                                <div className="flex items-center gap-2 truncate flex-1 min-w-0">
                                                  <Paperclip className="w-3.5 h-3.5 text-blue-500 flex-shrink-0" />
                                                  <div className="truncate text-left flex-1 min-w-0">
                                                    <p className="font-semibold truncate leading-tight text-[11px] group-hover:text-blue-600 transition-colors">{file.name}</p>
                                                    <p className="text-[9px] text-slate-400 font-mono mt-0.5">{file.size} • Click to Preview</p>
                                                  </div>
                                                </div>
                                                <span className="text-[9px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold shrink-0 ml-1">Preview</span>
                                              </div>
                                            )}
                                          </div>
                                        );
            ...
            <div className="flex items-center gap-2">
              {onDeleteCommunity && !["comm-1", "comm-2", "comm-3", "comm-4"].includes(community.id) && (currentUser.role === "platform_admin" || community.createdBy === currentUser.id) && (
                <button
                  type="button"
                  onClick={() => {
                    if (confirm(`Are you sure you want to delete community #${community.name}? All topics inside will be permanently deleted.`)) {
                      onDeleteCommunity(community.id);
                    }
                  }}
                  className="bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/60 dark:hover:bg-rose-900/60 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-800 text-xs font-bold px-3 py-2 rounded-xl transition flex items-center gap-1.5 cursor-pointer"
